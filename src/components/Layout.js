...
const Layout = ({ children }) => {
  // ... (Todo el estado y funciones existentes: sidebarOpen, showScanner, unreadNotifications, user, isAdmin, handleLogout, useEffect - sin cambios) ...
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className={`bg-gray-800 text-white w-64 ${sidebarOpen ? 'block' : 'hidden'} md:block flex-shrink-0`}>
        {/* ... (Contenido del Sidebar sin cambios) ... */}
         ...
         <nav className="mt-4">
          <ul>
            <li><Link to="/dashboard" className={`block py-2 px-4 ${location.pathname === '/dashboard' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Dashboard</Link></li>
            <li><Link to="/catalog" className={`block py-2 px-4 ${location.pathname === '/catalog' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Catálogo de Herramientas</Link></li>
             {isAdmin && (<li><Link to="/admin/borrowed-tools" className={`block py-2 px-4 ${location.pathname === '/admin/borrowed-tools' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Herramientas Prestadas</Link></li>)}
            {isAdmin && (<li><Link to="/admin/maintenance" className={`block py-2 px-4 ${location.pathname === '/admin/maintenance' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Herramientas en Mantenimiento</Link></li>)}
            <li><Link to="/my-tools" className={`block py-2 px-4 ${location.pathname === '/my-tools' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Mis Herramientas</Link></li>
            {isAdmin && (<li><Link to="/admin/tools" className={`block py-2 px-4 ${location.pathname === '/admin/tools' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Administrar Herramientas</Link></li>)}
            {isAdmin && (<li><Link to="/admin/users" className={`block py-2 px-4 ${location.pathname === '/admin/users' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}>Gestión de Usuarios</Link></li>)}
            {/* NUEVO ENLACE A REPORTES */}
        {isAdmin && (
          <li>
            <Link
              to="/admin/reports"
              className={`block py-2 px-4 ${location.pathname === '/admin/reports' ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
            >
              Reportes
            </Link>
          </li>
      ...
      {/* ======== CONTENIDO PRINCIPAL ======== */}
      {/* Usamos min-h-0 aquí para asegurar que el flex child pueda encoger si es necesario */}
      <div className="flex flex-col flex-1 overflow-hidden min-h-0">
        {/* ======== HEADER ======== */}
        <header className="bg-white shadow-md px-4 py-2 flex justify-between items-center flex-shrink-0">
          {/* ... (Contenido del Header sin cambios) ... */}
           <div className="flex items-center">
            <button className="md:hidden mr-2" onClick={() => setSidebarOpen(!sidebarOpen)}>☰</button>
            <h1 className="text-lg font-semibold">Taller Automotriz</h1>
          </div>
           <div className="flex items-center">
             <button onClick={() => setShowScanner(true)} className="mr-4 text-blue-500 hover:text-blue-700 flex items-center">
            ...
            <button onClick={handleLogout} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm">Cerrar sesión</button>
          </div>
        </header>

        {/* ======== CONTENIDO DE PÁGINA ======== */}
        {/* flex-1 y overflow-y-auto permiten que solo esta área haga scroll */}
        <main className="flex-1 overflow-y-auto p-4 bg-gray-100"> {/* Añadido bg-gray-100 aquí también */}
          {children}
        </main>

        {/* ======== NUEVO FOOTER ======== */}
        <footer className="bg-white px-4 py-3 border-t border-gray-200 text-center text-sm text-gray-600 flex-shrink-0">
          {/* Año dinámico y texto */}
          &copy; {new Date().getFullYear()} Rivera's Investments LLC. Todos los derechos reservados.
          {/* --> Reemplaza "[Sistema de Control de Herramientas]" con el nombre que prefieras <-- */}
        </footer>
      </div> {/* Fin del div flex-col */}

      {/* ======== MODALES Y OVERLAYS ======== */}
      {/* ... (Modal QR Scanner sin cambios) ... */}
       {showScanner && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 max-w-md w-full">
            <QRScanner onClose={() => setShowScanner(false)} />
            <div className="mt-4 flex justify-end">
              <button onClick={() => setShowScanner(false)} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Cancelar</button>
            </div>
          </div>
        </div>
      )}
    </div> // Fin del div flex h-screen
  );
};
